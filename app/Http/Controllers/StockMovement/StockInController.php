...
class StockInController extends BaseController
{
    ...
    public function index(Request $request)
    {
        ...
        return view('stock_movement.in', compact('goods', 'warehouses', 'locations'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $manufacturingDate = $request->manufacturing_date;
            ...
            return redirect()->route('stock_in')->with('success', '入庫登録が完了しました。');
        } catch (\Exception $e) {
            DB::rollBack();
            return redirect()->back()->with('error', '入庫登録に失敗しました: ' . $e->getMessage())->withInput();
        }
    }
}
